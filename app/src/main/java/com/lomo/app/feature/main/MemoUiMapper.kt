...
constructor(
    ...
    fun mapToUiModel(
        ...
        newContent = newContent.replace(Regex("(^|\\s)#[\\p{L}\\p{N}_][\\p{L}\\p{N}_/]*")) { match ->
        ...
        fun resolveImageModel(imageUrl: String, isWikiStyle: Boolean): Any {
            val cacheKey =
            ...
            if (basePath != null) {
                if (basePath.startsWith("content://") || basePath.startsWith("file://")) {
                    ...
                    return Uri.parse(basePath).buildUpon().appendPath(imageUrl).build().toString()
                }
