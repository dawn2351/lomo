...
constructor(
                    ...
                    try {
                        ...
                        val filesToDeleteInDb =
                                syncMetadataMap.filter { (key, _) ->
                                    ...
                                    if (isTrash) filename !in currentTrashFilenames
                                    else filename !in currentMainFilenames
                                }
                        ...
                        if (allMemos.isNotEmpty() || filesToDeleteInDb.isNotEmpty()) {
                            ...
                            if (allMemos.isNotEmpty()) {
                                ...
                                allMemos.forEach {
                                    updateMemoTags(it)
                                    val tokenized =
                                            com.lomo.data.util.SearchTokenizer.tokenize(it.content)
    ...
    private suspend fun syncFiles(
        ...
        if (allMemos.isNotEmpty()) {
            ...
            allMemos.forEach {
                updateMemoTags(it)
                val tokenized = com.lomo.data.util.SearchTokenizer.tokenize(it.content)
                dao.insertMemoFts(MemoFtsEntity(it.id, tokenized))
            }
        }
    }

    suspend fun saveMemo(content: String, timestamp: Long) =
            mutex.withLock {
                ...
                while (retryCount < maxRetries) {
                    ...
                    val filename =
                            DateTimeFormatter.ofPattern(filenameFormat)
                                    .withZone(zoneId)
                                    .format(instant) + ".md"
                    val timeString =
    ...
    suspend fun updateMemo(memo: Memo, newContent: String) =
            mutex.withLock {
                ...
                val tokenizedContent =
                        com.lomo.data.util.SearchTokenizer.tokenize(finalEntity.content)
    ...
    suspend fun restoreMemo(memo: Memo) =
            mutex.withLock {
                ...
                if (start != -1) {
                    ...
                    dao.insertMemo(entity)
                    updateMemoTags(entity)
                    val tokenized = com.lomo.data.util.SearchTokenizer.tokenize(entity.content)
    ...
    private suspend fun updateMemoTags(memo: MemoEntity) {
        val tags =
                if (memo.tags.isNotEmpty()) memo.tags.split(",").filter { it.isNotBlank() }
                else emptyList()
