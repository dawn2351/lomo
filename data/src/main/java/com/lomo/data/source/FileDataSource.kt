...
interface FileDataSource {
    ...
    suspend fun saveFile(filename: String, content: String, append: Boolean = false)
    suspend fun saveTrashFile(filename: String, content: String, append: Boolean = true)
...
data class FileMetadata(val filename: String, val lastModified: Long)
...
constructor(
    ...
    override fun getRootFlow(): Flow<String?> =
        combine(dataStore.rootUri, dataStore.rootDirectory) { uri, path -> uri ?: path }
    ...
    override fun getImageRootFlow(): Flow<String?> =
        combine(dataStore.imageUri, dataStore.imageDirectory) { uri, path -> uri ?: path }
    ...
    override suspend fun trashExists(filename: String): Boolean =
        getBackend()?.trashExists(filename) ?: false
    ...
    override suspend fun saveImage(uri: Uri): String =
        withContext(Dispatchers.IO) {
            ...
            when (backend) {
                ...
                null -> throw java.io.IOException("No image directory configured")
            }
    ...
    override suspend fun deleteImage(filename: String) {
        val (backend, _) = getImageBackend()
        when (backend) {
            is SafStorageBackend -> backend.deleteImage(filename)
